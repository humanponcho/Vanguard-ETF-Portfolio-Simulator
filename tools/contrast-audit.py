...
# shape:   foreground token -> surfaces, judged at 3:1 (marks, borders).
#
# This app has no quiz and no answered-row washes, so --wash-* are declared
# in the stylesheet but land nowhere yet. They are left out of the plan
# rather than tested against a surface they do not touch.
#
# The --series-* tokens are NOT checked here. A categorical chart palette is
# judged on perceptual separation between series, not on WCAG contrast against
# a surface, and three of the light steps deliberately sit below 3:1 under the
# relief rule (every slice carries a direct label). Validate that palette with
# the data-viz palette validator instead, not with this script.
SURFACES = ['--black', '--deep', '--panel', '--glass', '--g0', '--signal-note']
WASHES   = []

TEXT = {
    '--signal-ok':   ['--panel'],            # optimistic line label context
    '--signal-err':  ['--panel'],            # pessimistic line label context
    '--signal-warn': ['--panel'],            # disclaimer rule
    '--signal-note': ['--black', '--panel', '--glass', '--g0'],
    '--black':       ['--signal-note'],      # primary button label on the fill
...
SHAPE = {
    '--signal-ok':        ['--panel'],       # optimistic projection line
    '--signal-err':       ['--panel'],       # pessimistic line, disclaimer rule
    '--signal-warn':      ['--panel'],       # disclaimer rule
    '--signal-err-deep':  ['--g0'],          # declared; no adjacent-warn marks yet
    '--signal-note':      ['--panel', '--g0'],  # slider thumb, histogram bars
}
